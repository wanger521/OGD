...
class OursWorkerSoftmax(Softmax):
...
class OursServerSoftmax(Softmax):
    ...
    def trimmed_mean(self, remain, trimmed_number):
        trimmed_range = trimmed_number/len(remain)
        guess = stats.trim_mean(self.workerGrad, trimmed_range, axis=0)
        return guess
    ...
    def train(self):
        ...
        for i in range(self.config['byzantineSize']):
            trim_number = self.config['byzantineSize']-i
            # trim mean
            meanr = self.trimmed_mean(remain, trim_number)
